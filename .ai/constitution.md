<!--
SYNC IMPACT REPORT
- Version: 1.5.0 ÔåÆ 1.6.0  (added CA14 "raw-first, replayable ingestion"; CA4 cross-refs it)
...
- Numbering: CA1ÔÇôCA14, CC1ÔÇôCC5 (sub-steps CC1.1ÔÇôCC1.6), CW1ÔÇôCW9, CP1ÔÇôCP3, others per section
- Companion file: .specify/memory/requirements.md (+ requirements/S1ÔÇªS6) maps IDs to steps
- Added sections: none (CA14 added to Core Assumptions)
- Removed sections: none
- Templates requiring updates:
  ÔÜá .specify/templates/plan-template.md (not present ÔÇö add Constitution Check gate when introduced)
  ÔÜá .specify/templates/spec-template.md (not present)
  ÔÜá .specify/templates/tasks-template.md (not present)
- Follow-up TODOs:
...
> section ÔÇö `CG`, `CA`, `CC`, `CW`, `CD`, `CP`, `CT`, `CV`). Reference these IDs in
> plans, tests, commit bodies, and reviews. See `.specify/memory/requirements.md` for
> the per-step requirement mapping.

## Governance

- **CG1 ÔÇö** this constitution supersedes ad-hoc conventions. `AGENTS.md` and
  `.github/copilot-instructions.md` MUST stay consistent with it.
- **CG2 ┬À Amendments:** propose the change, state rationale and migration impact for
  existing data/formats, update dependent docs, then bump the version.
- **CG3 ┬À Versioning (semantic):** MAJOR = remove/redefine a principle or break
...
- **CG4 ┬À Compliance:** any PR/change touching data formats, endpoints, or config MUST
...
- **CG5 ┬À Highest priority, always-on:** this constitution and
  ...
  gates (S1 branch ÔåÆ S6 merge) MUST NOT be skipped, and changes cite the relevant IDs.
...
- **CA1 ┬À Simple first:** the obvious, readable solution wins. Complexity must earn its place.
- **CA2 ┬À Avoid libraries:** reach for plain PHP before any dependency (see *Procedural &
  Dependency-Free*, CP1). A new lib needs an explicit, documented justification.
- **CA3 ┬À Clean & non-breaking:** changes leave the tree tidy and never break existing
  data, formats, or endpoints (see *Data-Format Fidelity*, CP3 + Data & Compatibility
  Constraints).
- **CA4 ┬À Robust:** tolerate messy real-world input (mixed formats, wrapped quotes, odd
  dates); degrade gracefully, log, and keep serving valid rows (capture raw input first,
  see CA14).
- **CA5 ┬À Easy to debug & monitor:** prefer code paths that are observable through
  `log_*`/`log_return`; make failures visible, not silent.
- **CA6 ┬À Testable algorithms:** non-trivial logic (parsing, sorting, formatting,
...
- **CA7 ┬À Re-use over reinvent:** reuse existing patterns, methods, and concepts before
  adding new ones; share via `util_*.php`, don't copy-paste.
- **CA8 ┬À 80/20 by design:** optimize the common 80% path for simplicity and speed; allow
...
- **CA9 ┬À Prepare for caching, add later:** structure data access so a cache layer can be
...
- **CA10 ┬À Smart, minimal assumptions:** make explicit, documented assumptions when they
  simplify the 80% ÔÇö and isolate them so they're cheap to revisit.
- **CA11 ┬À Test-driven:** new behavior starts from a failing test; format/parse changes
  always ship with a `*_test.php` case (see The Basic Workflow ÔåÆ TDD, CW5).
- **CA12 ┬À Easy live-data capture:** make it trivial to capture real inputs ÔÇö end-to-end
  (e.g. `?format=txt.0.2` dumps) **and** per-function in-params/return values ÔÇö and
  replay them as fixtures in `*_test.php`.
- **CA13 ┬À Focused planning first:** invest in a good plan up front; define very focused
  tasks carrying only the context they need, and reuse known patterns aggressively.
- **CA14 ┬À Raw-first, replayable ingestion:** persist incoming input **verbatim and
  ...
  *Rationale: never lose a user's entry to a parser bug ÔÇö capture cheaply, process fallibly,
  reprocess after the fix.*

## Core Principles

### CC1 ┬À Assume Bugs Exist ÔÇö Reproduce, Document, Prove Fixes
Treat every defect as real and reproducible, not a one-off. The fix workflow is
mandatory: **(CC1.1) Find** ÔÇö capture the failing input/request and the observed vs.
expected behavior (use live-data capture, see Core Assumptions). **(CC1.2) Document** ÔÇö
record symptom, root cause, and affected formats/endpoints in the commit and, when
non-obvious, a short `// why` note. **(CC1.3) Re-run** ÔÇö turn the capture into a failing
`*_test.php` case that reproduces the bug (RED). **(CC1.4) Prove** ÔÇö make the test pass
...
**(CC1.6) Techniques** ÔÇö trace to the **root cause** (don't patch symptoms); add
**defense-in-depth** where a class of bug can recur; prefer **condition-based waiting**
over fixed `sleep()` in tests (poll the condition ÔÇö note `read.php`'s `ts` long-poll is
...
### CC2 ┬À Config-Driven, Never Hardcoded
Tunables are read as `$config['key'] ?? default` and declared in `infopedia.cfg`
under `[general]` plus the per-`$type` section (`entry`, `vote`, `download`, ÔÇª).
...
### CC3 ┬À Observability via Logging, Not Echo
...
### CC4 ┬À Fail Fast & Validate Input
Bad config or input stops the request immediately with `die("ÔÇª")`. Untrusted
identifiers are validated before use ÔÇö e.g. `tid` MUST match `[a-zA-Z0-9_-]{1,30}`,
...
### CC5 ┬À Self-Documenting Code, "Why" Comments
...
features ÔÇö but the order and gates hold.

1. **CW1 ┬À brainstorming** *(before writing code):* refine rough ideas through questions,
   explore alternatives, present the design in sections for validation. Save a design
   document.
2. **CW2 ┬À using-git-worktrees** *(after design approval):* create an isolated workspace on
   a new branch, run project setup, and verify a clean test baseline before any change.
3. **CW3 ┬À writing-plans** *(with approved design):* break work into bite-sized tasks
   (~2ÔÇô5 minutes each). Every task names exact file paths, the complete code, and
   verification steps.
4. **CW4 ┬À subagent-driven-development / executing-plans** *(with a plan):* dispatch a fresh
...
5. **CW5 ┬À test-driven-development** *(during implementation):* enforce RED ÔåÆ GREEN ÔåÆ
   REFACTOR ÔÇö write a failing test, watch it fail, write the minimal code, watch it
...
6. **CW6 ┬À requesting-code-review** *(between tasks):* review against the plan; report issues
   by severity. Critical issues block progress.
7. **CW7 ┬À receiving-code-review** *(after review):* triage every comment; fix it or push
   back with explicit rationale; re-review until the reviewer's critical issues are clear.
8. **CW8 ┬À verification-before-completion** *(before claiming done):* independently verify
   the work meets the spec ÔÇö run the full suite, exercise the actual change, and check the
   step gates. Never report success unverified.
9. **CW9 ┬À finishing-a-development-branch** *(when tasks complete):* verify tests, present
   options (merge / PR / keep / discard), and clean up the worktree.

## Data & Compatibility Constraints

- **CD1 ┬À Backward compatibility is non-negotiable:** existing `data/<tid>.csv|.cache|.log`
  and Google-Sheet exports MUST remain readable after any change.
- **CD2 ┬À Tenant isolation:** when `tid` is set, data lives in local `data/<tid>.*` files;
  Google is bypassed. Auto-creation only when `tenantAutoCreationEnabled=true`.
- **CD3 ┬À Format switch contract:** `read.php?format=` MUST keep emitting `csv` (default),
  ...
  char (`>!?.-`).
- **CD4 ┬À `infopedia.html` is the SPA shell (~4.9k lines):** edit surgically, never rewrite
...
### CP1 ┬À Procedural & Dependency-Free
...
- **CT1 ┬À Tests:** custom harness (no PHPUnit). `*_test.php` use `assert_equals`, `log_test`,
...
- **CT2 ┬À Local serving:** `start_http.bat` (`python -m http.server`) for the static shell.
- **CT3 ┬À Live data inspection:** `start_collect_all_txt_0_2.bat` (curl `?format=txt.0.2`).
- **CT4 ┬À Manual endpoint checks:** see header comments in `read.php` / `upload.php`, e.g.
  `/entry/get?sid=tst&tid=tenant1&force_update=1`.
- **CT5 ┬À Expected behavior:** `read.php` may `sleep()` up to ~50s in `ts` long-poll mode.

### Versioning & Commits

- **CV1 ┬À SemVer everywhere:** versioned artifacts (this constitution, data/format versions
  like `0v02`/`txt.0.2`/`json.0.3`, and any release tag) follow `MAJOR.MINOR.PATCH` ÔÇö
...
- **CV2 ┬À Conventional Commits:** every commit message uses `type(scope): summary` ÔÇö types:
...
- **CV3 ┬À Commit Ôåö SemVer link:** `fix:` ÔåÆ PATCH, `feat:` ÔåÆ MINOR, `!`/`BREAKING CHANGE:` ÔåÆ
